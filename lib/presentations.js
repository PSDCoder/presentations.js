            ...
            if (!$body.data(namespace + '-handlers')) {
                ...
                if (privateMethods.supportsFullscreen()) {
                    ...
                    $(document).on('webkitfullscreenchange mozfullscreenchange msfullscreenchange fullscreenchange', function (event) {
                        var fullScreenElement = privateMethods.getFullScreenElement();
                        var fullScreenClass = (fullScreenElement ? $(fullScreenElement) : $(event.target)).data(namespace).fullScreenClass;

                        $body[(fullScreenElement ? 'add' : 'remove') + 'Class'](fullScreenClass);
                    });
                }
