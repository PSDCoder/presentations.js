(function ($, undefined) {
    ...
    var privateMethods = {
        bindEventHandlers: function () {
            if (options.slideOnKeyboard) {
                $(window).on('keyup.' + namespace, function (event) {
                    ...
                    } else if (privateMethods.getFullScreenElement) {
            ...
            if (options.fullScreen && privateMethods.supportsFullscreen()) {
                $(window).on('keyup.' + namespace, function (event) {
                    ...
                    if (presentationsLength && presentationsLength < 2 && event.keyCode === options.fullScreenKey.toUpperCase().charCodeAt(0)) {
                        privateMethods.toggleFullScreen.call($(presentations[0]));
                    }
                });
            }
        },
        ...
        show: function (idOrNextPrevBool, callback) {
                ...
                for (var i = 0; i < slidesLength; i++) {
                    ...
                    if ($element.hasClass(options.activeSlideClass)) {
                        if (idOrNextPrevBool) {
                            ...
                            this.trigger('previousSlide', this, showIndex);
                        }
                        break;
                    }
                }
            ...
            if (showIndex === 0) {
                this.trigger('firstSlide', this, showIndex);
            } else if (showIndex === slidesLength - 1) {
                this.trigger('lastSlide', this, showIndex);
            }

            this.trigger('changeSlide', this, showIndex);
        ...
        toggleFullScreen: function () {
            ...
            if (fullScreenElement) {
                ...
                this.trigger('fullScreenExit', this);
            } else {
            ...
            $('body')[(fullScreenElement ? 'remove' : 'add') + 'Class'](options.fullScreenClass);
    ...
    var publicMethods = {
        init: function (passedOptions) {
            ...
            privateMethods.bindEventHandlers();
    ...
    if ($.fn[namespace]) {
        throw new Error('You trying include "' + namespace + '" plugin again.');
    }
